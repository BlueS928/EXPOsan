...
cleaning_chemicals_cost = 4 #$/m3
# Nayeong's paper https://pubs.acs.org/doi/10.1021/acsenergylett.3c00482 SI Equation 9, which cited from Generous et al. https://www.sciencedirect.com/science/article/pii/S1383586621005864?via%3Dihub
...
Centrifuge.ins[1].price = polymer_cost / 0.453592 * qs.CEPCI_by_year[2023] / qs.CEPCI_by_year[2014] #unit price of polymer in $/lb from CapdetWorks 2014 database, convert from lb to kg, not including Ferric Chloride cost

Microfiltration = su.Microfiltration('Microfiltration', ins = (Centrifuge-0, 'naocl', 'citric', 'bisulfite'), outs = ('mf_effluent', 'brine'))
BaseDosing = su.BaseDosing('BaseDosing', ins = (Microfiltration-0, 'base', AD_effluent.copy('AD_ref')), outs = ('bp_effluent',))
RedoxED = su.RedoxED('RedoxED', ins = (BaseDosing-0, AC_influent), outs = ('feeding_channel_out', 'accumulating_channel_out'), voltage = 0.8)
...
RedoxED.ins[0].price = cleaning_chemicals_cost # this calculates the cost of cleaning chemicals

### Creating system
sys = qs.System.from_units(ID = 'sys', units = [Centrifuge, Microfiltration, BaseDosing, RedoxED])
...
qs.StreamImpactItem(ID='centrif_sludge',
                    linked_stream=stream.solids_stream,
                    GlobalWarming=0.292)

qs.StreamImpactItem(ID='mf_brine_item',
                    linked_stream=stream.brine,
                    GlobalWarming=0.292)
...
qs.StreamImpactItem(ID='NaOCl_item',
                    linked_stream=stream.naocl,
                    GlobalWarming=1.2514)

qs.StreamImpactItem(ID='citric_acid_item',
                    linked_stream=stream.citric,
                    GlobalWarming=1.2514)

qs.StreamImpactItem(ID='bisulfite_item',
                    linked_stream=stream.bisulfite,
                    GlobalWarming=1.2514)
